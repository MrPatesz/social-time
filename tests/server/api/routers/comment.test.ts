                ...
                comments: DetailedCommentSchema.array().parse([
                    {
                        ...comment2,
                        user: user2,
                        event: { ...event1, location: location1, creator: user1, group: null },
                    },
                    {
                        ...comment3,
                        user: user2,
                        event: { ...event2, location: location2, creator: user2, group: null },
                    },
                ]),
                size: 2,
            };
    ...
    describe('create', () => {
        it('creates new comment for given event', async () => {
            ...
            const expectedComments = BasicCommentSchema.array().parse([
                { ...expectedNewComment, user: user2 },
                { ...comment3, user: user2 },
            ]);

            // Assert
            const mapComments = (cs: BasicCommentType[]) =>
                cs.map((c) => ({ ...c, id: undefined, postedAt: undefined }));
