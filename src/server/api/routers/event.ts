...
const utapi = new UTApi(); // TODO put this into context?
...
const filterDistantEvents = async <
    EventType extends {
        id: number;
    },
>(
    ...
    return maxDistance ?
            eventsWithDistance.filter((event) => event.distance < maxDistance)
        :   eventsWithDistance;
};
...
export const eventRouter = createTRPCRouter({
        ...
        .query(
            async ({
                ...
                const where = {
                    ...
                    OR: [{ groupId: null }, { group: { members: { some: { id: callerId } } } }],
                    start:
                        archive ?
                            {
                                lt: new Date(),
    ...
    getCalendar: protectedProcedure.output(BasicEventSchema.array()).query(
        async ({
            ctx: {
                session: {
                    user: { id: callerId },
            ...
            async ({
                ...
                await prisma.event.create({
                    data: {
                        ...
                        group:
                            createEvent.groupId ?
                                { connect: { id: createEvent.groupId } }
                            :   undefined,
                ...
                await pusher.trigger(
                    ...
                    InvalidateEvent.EventGetCalendar,
                    callerId
                );
                ...
                await pusher.trigger(
                    PusherChannel.INVALIDATE,
                    InvalidateEvent.EventGetPaginatedEvents,
                    null
                );
                await pusher.trigger(
                    PusherChannel.INVALIDATE,
                    InvalidateEvent.UserGetById,
                    callerId
                );
            }
        ),
        ...
        .mutation(
            async ({
                ...
                if (input.event.limit && participantsCount > input.event.limit) {
                    throw new TRPCError({
                        code: 'BAD_REQUEST',
                        message: 'Event has more participants than new limit!',
                    });
                }
                ...
                await prisma.event.update({
                    ...
                    data: {
                        ...
                        group:
                            input.event.groupId ?
                                { connect: { id: input.event.groupId } }
                            :   undefined,
                        creator: { connect: { id: callerId } },
                        location: {
                            connectOrCreate: {
    ...
    delete: protectedProcedure
        ...
        .mutation(
            async ({
                ...
                await Promise.all(
                    deletedEvent.images.map(async (image) => {
                        if (image) {
                ...
                await pusher.trigger(
                    PusherChannel.INVALIDATE,
                    InvalidateEvent.CommentGetAllByEventId,
                    input
                );
                await pusher.trigger(
                    PusherChannel.INVALIDATE,
