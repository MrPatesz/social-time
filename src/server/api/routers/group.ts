...
const calculateFreeIntervals = (
    events: Array<DateRange>,
    intervalStart: Date,
    intervalEnd: Date
) => {
...
export const groupRouter = createTRPCRouter({
    ...
    getFreeIntervals: protectedProcedure
        ...
        .query(
            async ({
                ...
                const allEvents = group.members.flatMap((member) => member.participatedEvents);

                const events = [...new Map(allEvents.map((item) => [item.id, item])).values()].sort(
    ...
    getJoinedGroups: protectedProcedure.output(BasicGroupSchema.array()).query(
        async ({
            ctx: {
                prisma,
                session: {
                    ...
                    await prisma.$transaction([
                        prisma.group.update({
                            ...
                            data: {
                                ...
                                creator: { connect: { id: callerId } },
                                members: {
                                    connect: joinRequests.map(({ userId }) => ({ id: userId })),
                                },
                            },
                        }),
