...
export const userRouter = createTRPCRouter({
    getPaginatedUsers: protectedProcedure
        ...
        .query(async ({ input: { page, pageSize, sortBy, searchQuery }, ctx: { prisma } }) => {
            const where =
                searchQuery ?
                    {
                        name: {
    ...
    profile: protectedProcedure.output(ProfileSchema).query(
        async ({
            ctx: {
                prisma,
                session: {
    ...
    deleteProfile: protectedProcedure.output(z.void()).mutation(
        async ({
            ctx: {
                prisma,
                session: {
            ...
            if (deletedUser.createdGroups.length) {
                ...
                await pusher.trigger(
                    PusherChannel.INVALIDATE,
                    InvalidateEvent.GroupGetPaginatedGroups,
                    null
                );
                await pusher.trigger(
                    PusherChannel.INVALIDATE,
                    InvalidateEvent.GroupChatGetMessages,
                    null
                );
            }
        }
    ...
    removeImage: protectedProcedure
        ...
        .mutation(
            async ({
                ...
                // could invalidate way more, but not worth it
                await pusher.trigger(
                    PusherChannel.INVALIDATE,
                    InvalidateEvent.UserGetById,
                    callerId
